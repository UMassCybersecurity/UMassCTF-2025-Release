...
typedef struct i2c_eeprom_t {
	...
	uint32_t reg_addr;		// read/write address register
	uint32_t size;				
	uint8_t addr_size; // also implies the address size, one or two byte
	uint8_t *ee;
} i2c_eeprom_t;
...
/*
 * Reset i2c internal data to a state of not being selected
 */
void
i2c_eeprom_reset(
	i2c_eeprom_t *p );
...
/**
 * Frees the memory allocated to the eeprom
 */
void
i2c_eeprom_cleanup(
		struct i2c_eeprom_t *p);
